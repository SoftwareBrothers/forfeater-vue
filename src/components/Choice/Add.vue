    ...
    <div v-if="orders.length">
      <div class="mt-3 d-flex flex-row justify-content-center">
        <div
          ...
          @click="choice = order.choice || {}"
        >
          <div class="card-header text-center">
            <h2>{{ order.vendor.name }}</h2>
            <small class="badge badge-warning">
              <Countdown
                v-if="order.deadlineAt"
                :end="order.deadlineAt"
              ></Countdown>
            </small>
          </div>
          ...
          <div
            v-if="isCancel(order)"
            class="card-footer bg-transparent border-warning"
          >
...
export default {
  ...
  async created() {
    ...
    if (!orders.length) {
      return this.$store.dispatch('setNotification', {
        type: 'warning',
        message: this.$t('orders.lack'),
        code: 200,
      });
    }
    this.orders = orders;
  ...
  methods: {
    sendForm: async function() {
      if (this.choice.product) {
        const choice = await this.service.save(
    ...
    isCancel(order) {
      return (
        (this.choice.order && this.choice.order.id) === order.id &&
        this.beforeDeadline(order)
      );
    },
  },
};
